...
Model catalogs are cached by **pi itself** (its provider models store, `models-store.json` in the agent dir): the extension serves the stored snapshot through `refreshModels` at startup and **publishes** a fresh snapshot there exactly once per real fetch (pi's legacy provider form does not persist returns on its own). Parallel pi instances sharing one agent dir coordinate on a `alibaba-catalog.lock` lockfile — one fetcher at a time, the rest skip and get the stored snapshot — plus a shared 10-minute freshness timestamp in `alibaba-config.json`. A failed fetch keeps the previous list (and, with none, registers an empty list rather than crashing). `/alibaba → Refresh model lists` forces a refresh through `ctx.modelRegistry.refresh({force: true})`.
...
- **Dynamic Caching**: Model lists live in pi's models store (offline snapshot at startup, freshness-gated network refresh). Many pi instances sharing one agent dir share that store — `models-store.json` is common and last-writer-wins — and the extension coordinates its catalog GETs with a `alibaba-catalog.lock` lockfile plus a shared 10-minute timestamp in `alibaba-config.json`: one instance fetches per window (and publishes one store write), the rest are served the snapshot. If a new model drops and you don't see it, run `/alibaba` -> `Refresh model lists` (force — always fetches).
...
Model catalogs are **not** written by the extension anymore — pi's provider models store keeps them (restored offline at startup); `alibaba-catalog.lock` is a transient fetch lock.
