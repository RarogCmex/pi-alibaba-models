...
- **Dynamic Caching**: Model lists live in pi's models store (offline snapshot at startup, freshness-gated network refresh). Many pi instances sharing one agent dir share that store — `models-store.json` is common and the extension additionally gates its own catalog GETs on a shared 10-minute timestamp in `alibaba-config.json`, so a fleet of parallel instances collapses to about one catalog fetch per window instead of one per launch. If a new model drops and you don't see it, run `/alibaba` -> `Refresh model lists` (force — always fetches).
