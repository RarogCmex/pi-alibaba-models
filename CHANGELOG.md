...
- **Many pi instances, one agent dir.** pi's `models-store.json` is shared and last-writer-wins (no cross-process locks), and the extension's own catalog GETs are now gated on a shared 10-minute timestamp in `alibaba-config.json`: one instance fetches per window, the rest are served pi's stored snapshot through `refreshModels`. `/alibaba → Refresh model lists` forces a fetch as before. The login seed no longer shadows a stored snapshot in the offline phase.
